...
| Tool           | Version / notes                                                          |
| -------------- | ------------------------------------------------------------------------ |
| Node.js        | 22.12 LTS or 24.x (`engines`: `>=22.12.0`; CI tests 22.12.0 and 24.21.0) |
...
├── MVP.md
├── itsa-ts-blueprint/     # R0 — this repo
├── itsa-dbms-base/        # R1
...
| ------------ | --------------------------------------- | ---------------------------------------------------- |
| `pre-commit` | `check-staged-file-limit` + lint-staged | Max **5 staged files** per commit; ESLint + Prettier |
| `commit-msg` | `commitlint`                            | Enforce Conventional Commits                         |
| `pre-push`   | `npm run check`                         | Match CI before push                                 |

Merge commits (e.g. GitHub PR merge) skip the staged-file limit via `MERGE_HEAD`.
